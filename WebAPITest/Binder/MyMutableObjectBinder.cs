...
{
    public class MyMutableObjectBinder : IModelBinder
    {

        internal static bool CanBindType(Type modetype)
        {
            if(TypeDescriptor.GetConverter(modetype).CanConvertFrom(typeof(string)))
            {
                return false;
            }
            if(modetype==typeof(ComplexModelDto))
            {
                return false;
            }
            return true;
